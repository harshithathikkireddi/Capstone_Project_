...
-- we only identified repeated customers and there are no new customers in the provided data 

select distinct name from category;

-- As we have 1000 different movies at our store and they are belongs to these 16 categories
/*
Action
Animation
Children
...
-- 3. Are there correlations between staff performance and customer satisfaction?
   ...
   -- there is no direct data related to staff performance and customer satisfaction but we can use proxy metrics and assumptions to identify relevant insights
...
/*
in the given database it is identified that all the films are available in english so this is a  limitatioin to analyse whether certain language films are more popular among specific customer segments
*/
...
/*
in Sakila database provided there is no information related to availability and knowledge of staff and also there is no customer ratings so for analysisng this question we require necessary data
...
/*
In the given database, there is no information about the age groups of the customers, so the above question cannot be answered
...
case when cte.no_of_copies-cte3.avg_weekly_rentals<=1 then 'Deficiency' 
	when cte.no_of_copies-cte3.avg_weekly_rentals>1 and cte.no_of_copies-cte3.avg_weekly_rentals<=3 then 'Adequate'
    else 'Surplus'
...
count(case when i.store_id = 1 then re.rental_id else null end) as influx_at_store_1,
count(case when i.store_id = 2 then re.rental_id else null end) as influx_at_store_2
from rental re
left join inventory i 
on re.inventory_id = i.inventory_id
group by 1
...
/*
There is no information related to customers' cultures and demographic factors although only the location of the customers are given in the dataset.
*/
...
/*
Presence of only english language films is the limitation to measure impact of languages on customer satisfaction and rental frequency
*/
